	...
	if ($line<2) {
		...
		if ($countD==0 and $countI>1){
			...
			if ($countD==0) {
				...
				$ref_forward=uc(substr($ref,($num_new[0]-$num_new[1]),$num_new[1]));
				$ref_backward=uc(substr($ref,($num_new[0]+$num_new[1]),$num_new[1]));
				...
				if ($num_new[1]==1 or $num_new[1]==2) {
					...
					##if the insertion is 1bp, the category is 2 or 13
                                        ##if the insertion is 2bp, the category is 11 or 12
                                        if ($num_new[1]==1){
                                                $category_1_1=2;
                                                $category_1_2=13;
                                        }elsif ($num_new[1]==2){
